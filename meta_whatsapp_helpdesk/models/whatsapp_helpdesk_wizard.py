...
class WhatsAppHelpdeskWizard(models.TransientModel):
    _name = 'whatsapp.helpdesk.wizard'
    _description = 'Wizard to Send WhatsApp Messages for Helpdesk Tickets'

    ticket_id = fields.Many2one(
        'helpdesk.ticket',
        string="Ticket",
        required=True,
        default=lambda self: self.env.context.get('default_ticket_id')
    )
    ...
    attach_pdf = fields.Boolean(
        string="Attach PDF",
        default=True,
        help="Automatically attach the ticket PDF."
    )
    ...
    def default_get(self, fields_list):
        """Override default_get to optionally attach a PDF or include ticket data in the message."""
        res = super(WhatsAppHelpdeskWizard, self).default_get(fields_list)
        ticket_id = res.get('ticket_id')
        attach_pdf = res.get('attach_pdf', False)
        if ticket_id:
            ticket = self.env['helpdesk.ticket'].browse(ticket_id)
            # Prepare ticket summary for the message
            ticket_summary = (
                f"Ticket: {ticket.name}\n"
                f"Customer: {ticket.partner_id.name}\n"
                f"Subject: {ticket.name if ticket.name else 'N/A'}\n"
                f"Priority: {ticket.priority if ticket.priority else 'N/A'}\n"
                f"Stage: {ticket.stage_id.name if ticket.stage_id else 'N/A'}\n"
                f"Team: {ticket.team_id.name if ticket.team_id else 'N/A'}"
            )
            if not res.get('message'):
                res['message'] = ticket_summary
            if attach_pdf:
                try:
                    report_type = res.get('report_type', 'custom')
                    if report_type == 'standard':
                        report_xml_id = 'helpdesk.action_report_helpdesk_ticket'  # Check if standard report exists in Enterprise
                        raise UserError(_('Standard report for Helpdesk tickets is not available in this configuration. Please use the custom report.'))
                    else:
                        report_xml_id = 'meta_whatsapp_helpdesk.action_report_custom_helpdesk_ticket'
                    ...
                    _logger.error("Failed to generate PDF for ticket %s: %s", ticket.id, str(e))
                    raise UserError(_('Failed to generate the ticket PDF: %s') % str(e))
        return res

    @api.depends('template_id', 'ticket_id')
    def _compute_message(self):
        """Compute the message field by reusing the logic from MessageConfiguration."""
        for wizard in self:
            if not wizard.template_id or not wizard.ticket_id:
            ...
            if not ticket_model:
                raise UserError(_('Model "helpdesk.ticket" not found in the system.'))
            ...
            message_config = self.env['message.configuration'].new({
                ...
                'model': ticket_model.id,
                'number_field': wizard.ticket_id.id,
            })
            ...
            try:
                ...
                _logger.error("Error calculating message for template %s in WhatsAppHelpdeskWizard: %s",
