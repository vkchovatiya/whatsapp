...
class WhatsAppInvoiceWizard(models.TransientModel):
    _name = 'whatsapp.invoice.wizard'
    _description = 'Wizard to Send WhatsApp Messages for Invoices'
    ...
    attach_pdf = fields.Boolean(
        string="Attach PDF",
        default=True,
        help="Automatically attach the invoice PDF."
    )
    ...
    def default_get(self, fields_list):
        """Override default_get to optionally attach a PDF or include invoice data in the message."""
        res = super(WhatsAppInvoiceWizard, self).default_get(fields_list)
        invoice_id = res.get('invoice_id')
        attach_pdf = res.get('attach_pdf', False)
        if invoice_id:
            invoice = self.env['account.move'].browse(invoice_id)
            # Prepare invoice summary for the message
            invoice_type = 'Invoice' if invoice.move_type == 'out_invoice' else 'Credit Note'
            invoice_summary = (
                f"{invoice_type}: {invoice.name}\n"
                f"Customer: {invoice.partner_id.name}\n"
                f"Date: {invoice.invoice_date}\n"
                f"Total Amount: {invoice.amount_total}\n"
                f"Due Date: {invoice.invoice_date_due or 'N/A'}\n"
                f"State: {invoice.state.capitalize()}"
            )
            if not res.get('message'):
                res['message'] = invoice_summary
            if attach_pdf:
                try:
                    ...
                    if report_type == 'standard':
                        ...
                        report_xml_id = 'meta_whatsapp_invoice.action_report_custom_invoice'
                    ...
                    _logger.error("Failed to generate PDF for invoice %s: %s", invoice.id, str(e))
                    raise UserError(_('Failed to generate the invoice PDF: %s') % str(e))
        return res

    @api.depends('template_id', 'invoice_id')
    def _compute_message(self):
        """Compute the message field by reusing the logic from MessageConfiguration."""
        for wizard in self:
            if not wizard.template_id or not wizard.invoice_id:
                wizard.message = False
                continue

            # Get the ir.model record for account.move
            invoice_model = self.env['ir.model'].search([('model', '=', 'account.move')], limit=1)
            if not invoice_model:
                raise UserError(_('Model "account.move" not found in the system.'))
            ...
            message_config = self.env['message.configuration'].new({
                ...
                'model': invoice_model.id,
                'number_field': wizard.invoice_id.id,
            })
            ...
            try:
                ...
                _logger.error("Error calculating message for template %s in WhatsAppInvoiceWizard: %s",
