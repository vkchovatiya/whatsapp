...
class WhatsAppPosWizard(models.TransientModel):
    _name = 'whatsapp.pos.wizard'
    _description = 'Wizard to Send WhatsApp Messages for POS Orders'

    pos_order_id = fields.Many2one(
        'pos.order',
        string="POS Order",
        required=True,
        default=lambda self: self.env.context.get('default_pos_order_id')
    )
    ...
    attach_pdf = fields.Boolean(
        string="Attach PDF",
        default=True,
        help="Automatically attach the POS order receipt PDF."
    )
    ...
    def default_get(self, fields_list):
        """Override default_get to optionally attach a PDF or include POS order data in the message."""
        res = super(WhatsAppPosWizard, self).default_get(fields_list)
        pos_order_id = res.get('pos_order_id')
        attach_pdf = res.get('attach_pdf', False)
        if pos_order_id:
            pos_order = self.env['pos.order'].browse(pos_order_id)
            # Prepare POS order summary for the message
            pos_summary = (
                f"POS Order: {pos_order.name}\n"
                f"Customer: {pos_order.partner_id.name}\n"
                f"Date: {pos_order.date_order}\n"
                f"Total: {pos_order.amount_total} {pos_order.currency_id.name}\n"
                f"Session: {pos_order.session_id.name}\n"
                f"State: {pos_order.state.capitalize()}"
            )
            if not res.get('message'):
                res['message'] = pos_summary
            if attach_pdf:
                try:
                    report_type = res.get('report_type', 'standard')
                    if report_type == 'standard':
                        report_xml_id = 'point_of_sale.sale_details_report'
                    else:
                        report_xml_id = 'meta_whatsapp_pos.action_report_custom_pos_order'
                    ...
                    _logger.error("Failed to generate PDF for POS order %s: %s", pos_order.id, str(e))
                    raise UserError(_('Failed to generate the POS order receipt PDF: %s') % str(e))
        return res

    @api.depends('template_id', 'pos_order_id')
    def _compute_message(self):
        """Compute the message field by reusing the logic from MessageConfiguration."""
        for wizard in self:
            if not wizard.template_id or not wizard.pos_order_id:
                wizard.message = False
                continue

            # Get the ir.model record for pos.order
            pos_model = self.env['ir.model'].search([('model', '=', 'pos.order')], limit=1)
            if not pos_model:
                raise UserError(_('Model "pos.order" not found in the system.'))
            ...
            message_config = self.env['message.configuration'].new({
                ...
                'model': pos_model.id,
                'number_field': wizard.pos_order_id.id,
            })
            ...
            try:
                ...
                _logger.error("Error calculating message for template %s in WhatsAppPosWizard: %s",
